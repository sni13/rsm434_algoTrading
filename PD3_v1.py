    ...
    while status == 'ACTIVE':        
        ...
        for i in range(3):
            if gross_position < MAX_EXPOSURE_GROSS:
                ...
                # outside of range, volume 5000
                if market_prices[i, 0] > estimates_data[i, 1]: 
                    resp1 = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker, 'type': 'MARKET', 'quantity': min(ORDER_LIMIT, allocate), 'price': market_prices[0, 0], 'action': 'SELL'})
                    resps.append(resp1)

                elif market_prices[i, 1] < estimates_data[i, 0]: 
                    resp2 = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker, 'type': 'MARKET', 'quantity': min(ORDER_LIMIT, allocate), 'price': market_prices[0, 1], 'action': 'BUY'})                
                    resps.append(resp2)
