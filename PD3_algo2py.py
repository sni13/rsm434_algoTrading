        ...
        for i in range(3):
            if gross_position < MAX_EXPOSURE_GROSS:
                ...
                if market_prices[i, 0] > estimates_data[i, 1]: 
                    ...
                    resp1 = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker, 'type': 'LIMIT', 'quantity': min(ORDER_LIMIT, out_volume), 'price': estimates_data[i, 1], 'action': 'SELL'})
                    resps.append(resp1)
                    ...
                    if abs(ticker_position) > 25000:
                        out_volume = min(out_volume // 2, 30000-abs(ticker_position))
                    resp2 = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker, 'type': 'LIMIT', 'quantity': min(ORDER_LIMIT, out_volume), 'price': estimates_data[i, 0], 'action': 'BUY'})                
                    resps.append(resp2)
                    ...
                    if ticker_position > 25000:
                        IF_BUY = False
                    elif ticker_position < -25000:
                        IF_SELL = False
                    
                    if abs(ticker_position) > 30000:
                        percent = 30
                        volume = 5000
                    elif abs(ticker_position) > 20000:
                        percent = 20
                        volume = 3000
                    elif abs(ticker_position) > 10000:
                        percent = 15
                        volume = 2000
                    else:
                        percent = 5
                    ...
                    if market_prices[i, 0] > (estimates_data[i, 1] - adj) and IF_SELL: #higher 10%
                        resp3 = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker, 'type': 'LIMIT', 'quantity': min(volume, allocate), 'price': market_prices[i, 0], 'action': 'SELL'})
                        resps.append(resp3)

                    elif market_prices[i, 1] < (estimates_data[i, 0] + adj) and IF_BUY:#lower 10%
                        resp4 = s.post('http://localhost:9999/v1/orders', params = {'ticker': ticker, 'type': 'LIMIT', 'quantity': min(volume, allocate), 'price': market_prices[i, 1], 'action': 'BUY'})                
                        resps.append(resp4)
        # sleep(0.5) 
